...
async function scrapeEquidiaVincennes() {
  ...
  for (let i = 0; i < MONTHS_AHEAD; i++) {
    ...
    try {
      ...
      if (!res.ok) { 
        console.warn(`⚠️ Erreur HTTP ${res.status} (${url})`); 
        continue; 
      }
      html = await res.text();
    } catch (err) {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        // 🚩 On ne prend que les courses à Vincennes
...
async function main() {
  ...
  if (races.length) {
    ...
    console.log("✅ Fichier static/races.json généré :", races.length, "courses Vincennes.");
  } else {
    console.warn("⚠️ Aucune course Vincennes trouvée sur Equidia.");
  }
}
