...
for target in TARGETS:
    ...
    # 🔧 Nouvelle méthode de récupération des stop_ids
    stop_ids = stops[stops['parent_station'] == parent_station]['stop_id'].tolist()

    if not stop_ids:
        stop_ids = stops[stops['stop_name'].str.contains(nom, case=False)]['stop_id'].tolist()

    if parent_station in stops['stop_id'].values:
        stop_ids.append(parent_station)

    stop_ids = list(set(stop_ids))  # Supprimer doublons

    print(f"▶ {nom} – {len(stop_ids)} arrêts trouvés pour {ligne}")
...
print("✅ Données GTFS exportées avec succès.")
